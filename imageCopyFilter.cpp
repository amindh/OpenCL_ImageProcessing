/**
 * \file imageCopyFilter.cpp
 * \mainpage Image filtering with openCL.
 *
 * This introduces openCL programming with images filtering
 *
...
int main(int argc, char* argv[])
{
  ...
  const char *filename = "filterImage.cl";
  string sourceStr;
  ...
  int errNum=0;
  cl_kernel kernel = clCreateKernel(program,"gauss2_filter", &errNum);
  if (kernel == NULL)
    {
...
//##################################  Partie modifiée ##########################################
  float sigma=50;
  // valeurs calculées pour la première et seconde optimisation
  ...
  // Charge dans la mémoire le tableau de flottants e
